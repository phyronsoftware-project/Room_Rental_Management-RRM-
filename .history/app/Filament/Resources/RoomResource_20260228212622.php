            ...
            Forms\Components\Grid::make(2)->schema([
                ...
                Forms\Components\Select::make('status')
                    ...
                    ->afterStateHydrated(function (Set $set, Get $get, $state, $record) {
                        // ✅ Auto force Occupied when this room has at least 1 tenant
                        if ($record && \App\Models\Tenant::query()->where('room_id', $record->room_id)->exists()) {
                            $set('status', 'Occupied');
                        }
                    })
                    ->disabled(function ($record) {
                        // ✅ Prevent manual changing away from Occupied while tenants exist
                        return $record && \App\Models\Tenant::query()->where('room_id', $record->room_id)->exists();
                    })
