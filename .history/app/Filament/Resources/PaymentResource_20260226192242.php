...
class PaymentResource extends Resource
{
    ...
    {
        // Property select (reactive/live depending on Filament version)
        ...
        if (method_exists($propertySelect, 'live')) {
            $propertySelect->live();
        } elseif (method_exists($propertySelect, 'reactive')) {
            $propertySelect->reactive();
        }

        // Room select (filtered by property)
        ...
        if (method_exists($roomSelect, 'live')) {
            $roomSelect->live();
        } elseif (method_exists($roomSelect, 'reactive')) {
            $roomSelect->reactive();
        }
            ...
            Forms\Components\Grid::make(2)
                ...
                ->schema([
                    ...
                    Forms\Components\TextInput::make('amount')
                        ...
                        ->disabled()
                        // ✅ IMPORTANT: still save to DB even if disabled
                        ->dehydrated(true)
                        ->required(),
